...
<body>
    ...
    <div class="info-section">
        <div class="info-row">
            ...
            <div class="info-value">{{ $startDate->format('F Y') }}</div>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
            ...
            <div class="info-value">{{ $monthlyStats['total_siswa'] }} orang</div>
        </div>
        <div class="info-row">
            <div class="info-label">Hari Sekolah:</div>
            <div class="info-value">{{ $monthlyStats['total_hari_sekolah'] }} hari</div>
        </div>
        <div class="info-row">
    ...
    <div class="stats-grid">
        <div class="stat-card">
            <div class="stat-number" style="color: #28a745;">{{ $monthlyStats['total_hadir'] + $monthlyStats['total_terlambat'] }}</div>
            <div class="stat-label">Total Kehadiran</div>
        </div>
        <div class="stat-card">
            <div class="stat-number" style="color: #17a2b8;">{{ $monthlyStats['total_izin'] }}</div>
            <div class="stat-label">Total Izin</div>
        </div>
        <div class="stat-card">
            <div class="stat-number" style="color: #6c757d;">{{ $monthlyStats['total_sakit'] }}</div>
            <div class="stat-label">Total Sakit</div>
        </div>
        <div class="stat-card">
            <div class="stat-number percentage {{ $monthlyStats['persentase_kehadiran'] >= 75 ? 'good' : 'bad' }}">
    ...
    <table class="summary-table">
        ...
        <tbody>
            <tr>
                <td>Hadir</td>
                <td>{{ $monthlyStats['total_hadir'] + $monthlyStats['total_terlambat'] }}</td>
                <td>
                    @php
                        $hadirPercentage = $monthlyStats['total_absen'] > 0 ? round((($monthlyStats['total_hadir'] + $monthlyStats['total_terlambat']) / $monthlyStats['total_absen']) * 100, 2) : 0;
                    @endphp
            ...
            <tr>
                <td>Izin</td>
                <td>{{ $monthlyStats['total_izin'] }}</td>
                <td>
                    @php
                        $izinPercentage = $monthlyStats['total_absen'] > 0 ? round(($monthlyStats['total_izin'] / $monthlyStats['total_absen']) * 100, 2) : 0;
                    @endphp
                    {{ $izinPercentage }}%
                </td>
            </tr>
            <tr>
                <td>Sakit</td>
                <td>{{ $monthlyStats['total_sakit'] }}</td>
                <td>
                    @php
                        $sakitPercentage = $monthlyStats['total_absen'] > 0 ? round(($monthlyStats['total_sakit'] / $monthlyStats['total_absen']) * 100, 2) : 0;
                    @endphp
                    {{ $sakitPercentage }}%
                </td>
            </tr>
            <tr style="font-weight: bold; background-color: #e9ecef;">
                <td>Total</td>
                <td>{{ $monthlyStats['total_absen'] }}</td>
                <td>100%</td>
            </tr>
        </tbody>
    </table>
    ...
    <div style="margin-top: 30px; padding: 15px; background-color: #f8f9fa; border-radius: 5px;">
        <h3 style="margin-top: 0;">Ringkasan Periode {{ $startDate->format('F Y') }}</h3>
        <ul style="margin: 10px 0; padding-left: 20px;">
            <li>Periode laporan: {{ $startDate->format('d F Y') }} - {{ $endDate->format('d F Y') }}</li>
            <li>Total hari sekolah efektif: {{ $monthlyStats['total_hari_sekolah'] }} hari</li>
            <li>Rata-rata kehadiran per hari: {{ $monthlyStats['total_hari_sekolah'] > 0 ? round(($monthlyStats['total_hadir'] + $monthlyStats['total_terlambat']) / $monthlyStats['total_hari_sekolah'], 1) : 0 }} siswa</li>
            <li>Persentase kehadiran keseluruhan:
                <span class="percentage {{ $monthlyStats['persentase_kehadiran'] >= 75 ? 'good' : 'bad' }}">
                    {{ $monthlyStats['persentase_kehadiran'] }}%
                </span>
            </li>
        </ul>
